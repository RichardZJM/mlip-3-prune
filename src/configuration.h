...
// Global function loads configurations from file, distribute them among processors and returns vector of scatered configurations on each processor (Does so with load balancing if available)
std::vector<Configuration> MPI_LoadBalanceCfgs(const std::string &filename, const int max_count = HUGE_INT);
...
inline void Configuration::resize(int new_size)
{
    ...
    if (new_size != size())
    {
        ...
        if (has_forces())
            forces_.resize(new_size);
        if (has_magmom())
            magmom_.resize(new_size);
        if (has_en_der_wrt_magmom())
            en_der_wrt_magmom_.resize(new_size);
        if (has_site_energies())
            site_energies_.resize(new_size);
        if (has_charges())
            charges_.resize(new_size);
        nbh_grades_.clear();
        ghost_inds.clear();
        ClearNbhs();
    }
