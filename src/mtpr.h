...
public:
    ...
    MLMTPR(const std::string &mtpfnm) : AnyLocalMLIP()
    {
        Load(mtpfnm);
    }
    ...
    int type_from_atomic_number(int a_num)
    { //!< returns the relative type (0,1,2,..) in the potential of the atomic number (33,47,..). Returns -1 if the atomic number is not present
