...
#include "../sample_potentials.h"
    ...
    BEGIN_COMMAND("train",
                  ...
                  "    --tolerance=<double>: stopping criterion for optimization. Default=1e-3\n"
                  "    --regularization=<double>: ridge (Tikhonov) parameter lambda. Default=1e-10, which is only large\n"
                  "        enough to keep the Cholesky solve stable; try 1e-8..1e-4 to actually regularize the fit.\n"
                  "        The per-species energy coefficients are intercepts and are exempt: they carry only a fixed\n"
                  "        1e-10 stability floor, so the fit stays invariant to the energy zero of the training set\n"
                  "    --regularization_mode=<relative or absolute>: relative (default) adds lambda*max(1,H_ii) to each\n"
                  "        diagonal entry of the SLAE, which is equivalent to uniform Tikhonov in the Jacobi-scaled space,\n"
                  "        so lambda is dimensionless and invariant to rescaling of individual basis functions.\n"
                  "        absolute applies a uniform lambda per configuration and disables the adaptive update;\n"
                  "        it requires --regularization to be given explicitly\n"
    ...
    BEGIN_COMMAND("profile_costs",
                  "Profiles gradient evaluation costs for each configuration in a dataset.",
                  "mlp profile_costs pot.mtp dataset.cfg sample_count\n"
                  "  sample_count: Number of iterations to average per configuration.\n")
    {
        if (args.size() != 3)
        {
            cout << "mlp profile_costs: 3 arguments are required (pot, dataset, sample_count)\n";
            return 1;
        }

        string pot_fnm = args[0];
        string db_fnm = args[1];
        int sample_count = std::stoi(args[2]);
        string out_fnm = db_fnm + ".profiled";

        if (sample_count <= 0)
        {
            cout << "sample_count must be a positive integer.\n";
            return 1;
        }
