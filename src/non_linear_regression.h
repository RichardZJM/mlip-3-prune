...
protected:
    ...
    Matrix3 dLdS;
    void AddLoss(Configuration &orig, bool penalize = true);
...
public:
    ...
    NonLinearRegression(AnyLocalMLIP *_p_mlip, const Settings &_settings, const bool verbose = true) : AnyTrainer(_p_mlip, _settings, verbose) {};

    double ObjectiveFunction(std::vector<Configuration> &train_set, bool penalize = true); // Calculates objective function summed over train_set
