        ...
        for (int s1 = 0; s1 < species_count; s1++) // loads the radial coefficients for each pair of atomic species
            for (int s2 = 0; s2 < species_count; s2++)
            {
                ...
                for (int i = 0; i < radial_func_count; i++)
                {
                    ...
                    { // reading the coefficients from file
...
void MLMTPR::CalcEFSComponents(Configuration &cfg)
{
    ...
    // SWAPPED: Make 'n' the innermost (contiguous) dimension
...
void MLMTPR::CalcBasisFuncsDers(const Neighborhood &nbh)
{
    ...
    for (int j = 0; j < nbh.count; j++)
    {
        ...
        for (int i = 0; i < alpha_index_basic_count; i++)
        {
            ...
            {
                moment_ders(i, j, 0) += val * alpha_index_basic[i][1] * coords_powers_[alpha_index_basic[i][1] - 1][0] * pow1 * pow2;
            }
            if (alpha_index_basic[i][2] != 0)
            {
                moment_ders(i, j, 1) += val * alpha_index_basic[i][2] * pow0 * coords_powers_[alpha_index_basic[i][2] - 1][1] * pow2;
            }
            if (alpha_index_basic[i][3] != 0)
            {
                moment_ders(i, j, 2) += val * alpha_index_basic[i][3] * pow0 * pow1 * coords_powers_[alpha_index_basic[i][3] - 1][2];
            }
        }
    }
    // Next: calculating non-elementary b_i
...
MLMTPR::~MLMTPR()
{
    if (alpha_moment_mapping != NULL)
        delete[] alpha_moment_mapping;
    if (alpha_index_times != NULL)
        delete[] alpha_index_times;
    if (alpha_index_basic != NULL)
...
void MLMTPR::CalcSiteEnergyDers(const Neighborhood &nbh)
{
    ...
    for (int j = 0; j < nbh.count; j++)
    {
        ...
        for (int i = 0; i < alpha_index_basic_count; i++)
        {
            ...
            if (alpha_index_basic[i][1] != 0)
            {
                moment_jacobian_(i, j, 0) += val * alpha_index_basic[i][1] * coords_powers_[alpha_index_basic[i][1] - 1][0] * pow1 * pow2;
            }
            if (alpha_index_basic[i][2] != 0)
            {
                moment_jacobian_(i, j, 1) += val * alpha_index_basic[i][2] * pow0 * coords_powers_[alpha_index_basic[i][2] - 1][1] * pow2;
            }
            if (alpha_index_basic[i][3] != 0)
            {
                moment_jacobian_(i, j, 2) += val * alpha_index_basic[i][3] * pow0 * pow1 * coords_powers_[alpha_index_basic[i][3] - 1][2];
            }
        }
    ...
    // if (wgt_eqtn_forces != 0) //!!! CHECK IT!!!
    {
        ...
        for (int i = alpha_index_times_count - 1; i >= 0; i--)
        {
            ...
            site_energy_ders_wrt_moments_[alpha_index_times[i][1]] +=
                site_energy_ders_wrt_moments_[alpha_index_times[i][3]] * val2 * val0;
            site_energy_ders_wrt_moments_[alpha_index_times[i][0]] +=
                site_energy_ders_wrt_moments_[alpha_index_times[i][3]] * val2 * val1;
        }
...

    {
        ...
        for (int j = 0; j < nbh.count; j++)
        {
            ...
            for (int i = 0; i < alpha_index_basic_count; i++)
            {
                    ...
                    for (int gamma = 0; gamma < M; gamma++)
                    {
                        ...
                        for (int xi = 0; xi < R; xi++)
                        {
                            ...
                            if (se_ders_weights != nullptr)
                            {
                                ...
                                if (alpha_index_basic[i][1] != 0)
                                {
                                    derx += rad_val_mag_val_mag_val * powk * alpha_index_basic[i][1] * auto_coords_powers_[alpha_index_basic[i][1] - 1][0] * pow1 * pow2;
                                }

                                if (alpha_index_basic[i][2] != 0)
                                {
                                    dery += rad_val_mag_val_mag_val * powk * alpha_index_basic[i][2] * pow0 * auto_coords_powers_[alpha_index_basic[i][2] - 1][1] * pow2;
                                }

                                if (alpha_index_basic[i][3] != 0)
                                {
                                    derz += rad_val_mag_val_mag_val * powk * alpha_index_basic[i][3] * pow0 * pow1 * auto_coords_powers_[alpha_index_basic[i][3] - 1][2];
                                }
                ...
                if (alpha_index_basic[i][1] != 0)
                {
                    mom_jacobian_(i, j, 0) += val * alpha_index_basic[i][1] * auto_coords_powers_[alpha_index_basic[i][1] - 1][0] * pow1 * pow2;
                }
                if (alpha_index_basic[i][2] != 0)
                {
                    mom_jacobian_(i, j, 1) += val * alpha_index_basic[i][2] * pow0 * auto_coords_powers_[alpha_index_basic[i][2] - 1][1] * pow2;
                }
                if (alpha_index_basic[i][3] != 0)
                {
                    mom_jacobian_(i, j, 2) += val * alpha_index_basic[i][3] * pow0 * pow1 * auto_coords_powers_[alpha_index_basic[i][3] - 1][2];
                }
            }
            // Repulsive term
            if (p_RadialBasis->GetRBTypeString() == "RBChebyshev_repuls")
